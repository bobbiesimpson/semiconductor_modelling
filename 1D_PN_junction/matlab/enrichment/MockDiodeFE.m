...
SUPG=false;
bubble=true;
groupedFE=true;        %   are we using the Grouped FE (equiv to FD for poisson eqn)
enriched=false;
...
%             
        ...
        if doWePlot
            figure(10); hold on;
            plot(meshcoords, current_p, 'ko-'); hold off;
            ...
            psiExport=[meshcoords' currentPsi]; elecExport=[meshcoords' current_n];
            holeExport=[meshcoords' current_p];
            potString='dat_files/unstabilisedResultsFE/unstablePotentialMock.dat';
            elecString='dat_files/unstabilisedResultsFE/unstableElecConcMock.dat';
            holeString='dat_files/unstabilisedResultsFE/unstableHoleConcMock.dat';
...
if doWePlot
   ...
   figure(2); hold on; loglog(DOF, L2psi_mesh, 'ro--'); hold off;
   figure(3); hold on; loglog( DOF, L2n_mesh,'ro--'); hold off;
   figure(4); hold on; loglog( DOF,L2p_mesh, 'ro--'); hold off;

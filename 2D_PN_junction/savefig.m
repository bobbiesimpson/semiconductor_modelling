...
% '-painters': Use painters to render the figure using vectors. Useful for
%              low number of objects
% '-opengl': use opengl to render the figure using bitmaps. For big figs.
%
% EXAMPLE:
...
%	device.png.cmyk=	sprintf(cmdEnd,	'png???', 					[fname '.png']);
	...
	for n= 1:length(varargin)											% Read options.
		if(ischar(varargin{n}))
            ...
            case '-painters',            forcerenderer=  '-painters';
            case '-opengl',              forcerenderer=  '-opengl';    
			otherwise

...
class Poisson:
	...
	#device parameters
	dx= 1e-7
	tdev=80e-7					# ie the length of the device is 100 nm or 100e-7cm
	...
	while error>tol:
	    ...
	    for mp in mesh:
		    ...
		    print 'electron conc. ', mp.cn, ' and hole conc. ',mp.cp, 'and disc_rhs ',mp.disc_rhs
		    print 'Middle node disc: ',mp.disc_m, ' and jacobian ',bj
        	    d.append(-bb+bj*mp.p)
	...
	#xcoord, FEpsi = FEpoisson1D.main(acceptors, vg, False)		#	run the FE model
	#xcoord = xcoord * 1e7
	# plot 
	clf()
	plot(x, y, 'bo-', label='Finite difference')
	#plot(xcoord, FEpsi, 'kx-', label='Finite element')
	xlabel('distance (nm)')
	ylabel('potential (V)')
	legend()
	savefig('FE_2GP_ConvVsFD.eps')
	show()
